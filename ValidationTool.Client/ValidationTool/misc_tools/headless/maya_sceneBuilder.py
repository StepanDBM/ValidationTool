...
def add_uv_chaos(obj):
    ...
    for i in range(1, 5):
        ...
        existing = cmds.polyUVSet(obj, q=True, allUVSets=True) or []
        if not existing:
            continue
...
def build_test_scene():
    ...
    cmds.setAttr(f"{env1}.scaleX", 2000)
    ...
    cmds.setAttr(f"{mod}.scaleX", -3)
    cmds.setAttr(f"{mod}.scaleY", 2000)
    cmds.setAttr(f"{mod}.scaleZ", 0)
    add_uv_chaos(mod)
