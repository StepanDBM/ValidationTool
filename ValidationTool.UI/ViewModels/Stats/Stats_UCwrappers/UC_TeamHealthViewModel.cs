...
namespace ValidationTool.UI.ViewModels.StatsView_Models {
    public class UC_TeamHealthViewModel : ViewModelBase {
        public ObservableCollection<TeamHealthItemViewModel> Items { get; } =
            new ObservableCollection<TeamHealthItemViewModel>();

        private string _subtitle = "No team health information loaded.";
        public string Subtitle {
            get => _subtitle;
            set => SetProperty(ref _subtitle, value);
        }

        private int _displayedTeamCount;
        public int DisplayedTeamCount {
            get => _displayedTeamCount;
            set => SetProperty(ref _displayedTeamCount, value);
        }
        ...
        public void Apply(IEnumerable<StatsTeamHealthItem> items) {
            Items.Clear();

            if (items == null) {
                DisplayedTeamCount = 0;
                MaxSeverityScore = 0;
                TotalIssues = 0;
                Subtitle = "No team health information loaded.";
                return;
            }
            ...
            foreach (var item in list) {
                ...
                Items.Add(new TeamHealthItemViewModel {
                    Team = item.Team,
                    ...
                    ArtistCountDisplay = item.ArtistCount.ToString("N0", CultureInfo.InvariantCulture),
            ...
            Subtitle = DisplayedTeamCount > 0
                ? $"Top {DisplayedTeamCount.ToString("N0", CultureInfo.InvariantCulture)} teams ranked by weighted severity density."
                : "No team health information loaded.";
        }
    }

    public class TeamHealthItemViewModel : ViewModelBase {
        ...
        public int ArtistCount {
            get => _artistCount;
            set => SetProperty(ref _artistCount, value);
        }

        private string _artistCountDisplay;
        public string ArtistCountDisplay {
            get => _artistCountDisplay;
            set => SetProperty(ref _artistCountDisplay, value);
        }
